...
//    @Captor // @Captor: Simplifies capturing arguments passed to methods.
//    ArgumentCaptor<User> captor = ArgumentCaptor.forClass(User.class); //ArgumentCaptor: Verifies methods were called w/ specific arguments.
    ...
    void setUp() {
        ...
        // "Whenever saveUser() method is called on userService (with any User object as input), return userAccount3."
        when(userService.saveUser(any(User.class))).thenReturn(userAccount3);
    }
    ...
    void shouldSaveANewUser() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/user") // Simulates POST request to endpoint /api/v1/user.
                ...
                .andExpect(status().isCreated()); // Checks that HTTP response status is 201 created.

        // Assert - Meaning: Verify the result is correct. What you do here: Check if the output matches expectations.
        verify(userService, times(1)).saveUser(any(User.class)); // Confirms that the controller called userService.saveUser() exactly once.
    }
